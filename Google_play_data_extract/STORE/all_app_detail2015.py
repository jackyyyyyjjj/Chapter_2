...
cur = sqlite.connect('appmonsta_app.sqlite')  # you can change the databse name here
conn = cur.cursor()
...
# for i in range(7):
#     with open('./%s.json'%(name_list[i]), encoding='utf-8') as f:
#         count += 1
#         while True:
#             line = f.readline()
#             count += 1
...
with open('AppMonstaPlayStoreDetails20150817.json', encoding='utf-8') as f:
    count += 1
    while True:
        line = f.readline()
        count += 1
    ...
    # Load json object and print it out
        json_record = json.loads(line)
        columns = make_column_args(json_record)
        tablename = 'AppMonstaPlayStoreDetails20150817'
        date = '2015-08-17'
        post_row(conn, tablename, json_record, columns, date)
    # count how many apps have been stored into the database
        if count % 1000 == 0:
