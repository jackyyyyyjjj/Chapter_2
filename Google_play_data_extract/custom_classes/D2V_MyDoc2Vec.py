...
class MyDoc2Vec:
    ...
    def compare_similar(self, input_doc:list,  custom_stopwords:list):
        """
        ...
        tokens1 = self.clean_text(text = str1,
                                 tokenizer = word_tokenize,
                                 stopwords = custom_stopwords)
        print(f"Tokens from input text: {tokens1}")
        tokens2 = self.clean_text(text = str2,
                                 tokenizer = word_tokenize,
                                 stopwords = custom_stopwords)
        print(f"Tokens from input text: {tokens2}")
        # Create vector representation from tokens TRANSFER THE NEW WORD INTO A VECTOR
        text_vector_represented1 = self.doc2vec_model.infer_vector(tokens1, epochs=300, alpha=0.025)
        text_vector_represented2 = self.doc2vec_model.infer_vector(tokens2, epochs=300, alpha=0.025)
        print(f"Shape of text in vector representation: {text_vector_represented1.shape}")
        print(f"Shape of text in vector representation: {text_vector_represented2.shape}")
        ...
        cos_distance = spatial.distance.cosine(text_vector_represented1, text_vector_represented2)
        print(cos_distance)
        return cos_distance
