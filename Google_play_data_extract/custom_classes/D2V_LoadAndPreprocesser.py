...
class LoadAndPreprocessor:
    ...
    def __init__(self, PATH_TO_DATA:str, TARGET_COLUMN:str, sep=',', type='sqlite'):
        """
        ...
        if type=='sqlite':
            with sqlite3.connect(PATH_TO_DATA) as con:
                c = con.cursor()
                # self.master = pd.read_sql('SELECT A.id,A.publisher_url,A.publisher_id,A.publisher_email,A.publisher_name,'
                #                           'B.id,B.publisher_url,B.publisher_id,B.publisher_email,B.publisher_name from AppMonstaPlayStoreDetails20150817 A '
                #                           'JOIN AppMonstaPlayStoreDetails20160815 B ON A.id = B.id', con=con)
                self.master = pd.read_sql('SELECT A.id,A.publisher_url,A.publisher_id,A.publisher_email,A.publisher_name,'
                                          'B.id,B.publisher_url,B.publisher_id,B.publisher_email,B.publisher_name from AppMonstaPlayStoreDetails20200817 A '
                                          'JOIN AppMonstaPlayStoreDetails20210816 B ON A.id = B.id', con=con)
                self.master.columns = ['A.id','A.publisher_url','A.publisher_id','A.publisher_email','A.publisher_name','B.id','B.publisher_url','B.publisher_id','B.publisher_email','B.publisher_name']
                self.master['publisher_name'] = self.master['A.publisher_name'].astype(str).map(str) + '-' +  self.master['B.publisher_name'].astype(str).map(str)
                self.master['IndexN'] = np.arange(len(self.master.iloc[:,0]))
    ...
    def do_sentence_split(self, column:str, set_to_target_column:bool, name_of_new_sentence_column:str):
        ...
        self.master = self.master.explode(name_of_new_sentence_column, ignore_index = True)#使用一个字符串分割另一个字符串，并返回由字符串组成的数组
        
        # Set to target columndo_sentence_split
